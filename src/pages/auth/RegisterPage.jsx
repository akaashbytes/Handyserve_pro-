...
import { SERVICE_CATEGORIES, LOCATIONS } from '../../data/mockData';
...
export default function RegisterPage() {
  ...
  const validate = () => {
    if (!formData.name || !formData.email || !formData.phone || !formData.password || !formData.state || !formData.city || !formData.location) return 'Please fill all required fields.';
    ...
    <div style={{
      ...
      <div style={{
        ...
        <Link to="/" style={{ 
          ...
          <span style={{ background: 'var(--brand)', color: '#fff', width: 32, height: 32, borderRadius: '8px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '16px' }}>🔧</span>
          HandyServe
        </Link>
              ...
              <form onSubmit={handleRegister}>
                <div style={{ display: 'grid', gridTemplateColumns: role === 'provider' ? '1fr 1fr' : '1fr', gap: '32px' }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
                    <Card padding="28px">
                      <h3 style={{ fontSize: '16px', fontWeight: 700, marginBottom: '20px', color: 'var(--text-primary)', borderBottom: '1px solid var(--border)', paddingBottom: '10px' }}>Location Details</h3>
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                          <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                             <label style={{ fontSize: '13px', fontWeight: 600, color: 'var(--text-secondary)' }}>State *</label>
                             <select name="state" value={formData.state} onChange={(e) => setFormData({...formData, state: e.target.value, city: '', location: ''})} style={selectStyle}>
                          ...
                          <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                             ...
                             <select name="city" value={formData.city} disabled={!formData.state} onChange={(e) => setFormData({...formData, city: e.target.value, location: ''})} style={selectStyle}>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                          <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                             <label style={{ fontSize: '13px', fontWeight: 600, color: 'var(--text-secondary)' }}>Area *</label>
                             <select name="location" value={formData.location} disabled={!formData.city} onChange={(e) => setFormData({...formData, location: e.target.value})} style={selectStyle}>
                                <option value="">Select Area</option>
                                {areas.map(a => <option key={a} value={a}>{a}</option>)}
                             </select>
                          </div>
                          <Input label="Pincode" name="pincode" value={formData.pincode} onChange={handleChange} placeholder="600040" />
                        </div>
                        <Input label="Full Address *" name="address" value={formData.address} onChange={handleChange} placeholder="House No, Street name" />
                      </div>
                    </Card>
