              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                <div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '4px' }}>
                    <h3 style={{ fontSize: '16px', fontWeight: 700, color: 'var(--text-primary)' }}>{req.firstName} {req.lastName}</h3>
                    <Badge color={req.status === 'pending' ? 'warning' : 'success'}>
                      {req.status === 'pending' ? 'Pending' : 'Resolved'}
                    </Badge>
                  </div>
                  <p style={{ fontSize: '13px', color: 'var(--text-secondary)' }}>📧 {req.email} • 📅 {req.date}</p>
                </div>
                {req.status === 'pending' && (
