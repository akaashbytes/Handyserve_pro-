  ...
  return (
    <div style={{ padding: '32px 36px', maxWidth: '1000px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: '16px', marginBottom: '24px' }}>
        {[
          { label: 'Open Disputes',     count: disputes.filter(d => d.status === 'Open').length,     color: 'var(--danger)',  icon: '🔴' },
          ...
          <Card key={s.label} padding="20px">
            <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
              <div style={{ width: 46, height: 46, borderRadius: 'var(--radius-md)', background: 'var(--bg-elevated)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '22px' }}>{s.icon}</div>
              <div>
          ...
          <Card key={d.id} padding="22px">
            <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '16px', flexWrap: 'wrap' }}>
              <div style={{ display: 'flex', gap: '14px', flex: 1 }}>
                <div style={{ width: 46, height: 46, borderRadius: 'var(--radius-md)', background: 'var(--danger-light)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '22px', flexShrink: 0 }}>⚠️</div>
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ display: 'flex', gap: '16px', flexWrap: 'wrap' }}>
                    <p style={{ fontSize: '12.5px', color: 'var(--text-secondary)' }}>👤 Customer: <strong>{d.customer}</strong></p>
                    {d.customerEmail && <p style={{ fontSize: '12.5px', color: 'var(--text-secondary)' }}>✉️ {d.customerEmail}</p>}
                    <p style={{ fontSize: '12.5px', color: 'var(--text-secondary)' }}>🔧 Provider: <strong>{d.provider}</strong></p>
                    <p style={{ fontSize: '12.5px', color: 'var(--text-secondary)' }}>📅 {d.date}</p>
                  </div>
                </div>
              </div>
