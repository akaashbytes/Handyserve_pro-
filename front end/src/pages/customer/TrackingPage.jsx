...
export default function TrackingPage() {
  ...
  const activeBooking = bookings.find(b => ['Requested', 'Accepted', 'In Progress', 'Pending Payment'].includes(b.status));
  ...
  const steps = [
    ...
    { label: 'Accepted',    icon: '✅', done: ['Accepted', 'In Progress', 'Pending Payment', 'Completed'].includes(activeBooking.status), active: activeBooking.status === 'Accepted' },
    { label: 'On the way',  icon: '🚗', done: ['In Progress', 'Pending Payment', 'Completed'].includes(activeBooking.status), active: false },
    { label: 'In Progress', icon: '🔧', done: ['Pending Payment', 'Completed'].includes(activeBooking.status), active: activeBooking.status === 'In Progress' },
    { label: 'Completed',   icon: '🎉', done: activeBooking.status === 'Completed', active: activeBooking.status === 'Pending Payment' },
  ];
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '18px' }}>
          ...
          <Card padding="0" style={{ overflow: 'hidden', height: 320, borderRadius: 20 }}>
            <div style={{ height: '100%', position: 'relative' }}>
              ...
              <div style={{ position: 'absolute', right: 12, top: 12, maxWidth: 220, background: 'var(--bg-card)', padding: '12px 14px', borderRadius: 12, boxShadow: 'var(--shadow-md)', border: '1.5px solid var(--border)', zIndex: 1000 }}>
                {activeBooking.status === 'Pending Payment' ? (
                  <>
                    <p style={{ fontWeight: 700, color: 'var(--success)', fontSize: 13.5, margin: 0 }}>Job Completed!</p>
                    <p style={{ fontSize: 12, color: 'var(--text-secondary)', margin: '6px 0' }}>Payment is pending for this booking.</p>
                    <Button size="sm" fullWidth onClick={() => navigate('/customer/payments')} style={{ fontSize: 11, padding: '6px 12px' }}>Pay Now 💳</Button>
                  </>
                ) : !showProviderOnMap ? (
                  <p style={{ fontSize: 12, color: 'var(--text-secondary)', margin: 0 }}>Live provider location appears after they accept.</p>
                ) : (
                  <>
